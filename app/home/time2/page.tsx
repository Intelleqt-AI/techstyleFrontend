...
function getFormattedTimeFromMondayToSaturday(tasks) {
  ...
  return tasks
    .reduce((totalHours, task) => {
      // 1. Check if task was worked on this week
      const taskDate = new Date(task.timerStart);
      if (taskDate < monday || taskDate > saturday) {
        return totalHours;
      }
      ...
      if (Array.isArray(task.session)) {
        sessionTime = task.session.reduce((sum, session) => {
          const sessionDate = new Date(session.date);
          if (sessionDate >= monday && sessionDate <= saturday) {
            if (session.endTime) {
              // Completed session - always count
              return (
                sum + (Number(session.endTime) - Number(session.startTime))
...
function getDailyBreakdown(tasks: any[]): Day[] {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    dayStart.setDate(monday.getDate() + (i - 1)); // Adjust since we started with Monday

    const dayEnd = new Date(dayStart);
    dayEnd.setHours(23, 59, 59, 999);

    const dayName = daysOfWeek[(1 + i) % 7]; // Start with Monday

    // Calculate hours for this day
    const dayHours = tasks.reduce((totalHours, task) => {
      // 1. Check if task was worked on this day
      const taskDate = new Date(task.timerStart);
    ...
    dailyHours.push({
      day: dayName,
      hours: parseFloat(dayHours.toFixed(1)), // Round to 1 decimal
    });
  }
              ...
              <div className="space-y-0.5">
                ...
                <div className="text-sm text-neutral-500">
                  {activeTask?.task?.projectID
                    ? `Project ID: ${activeTask.task.projectID}`
                    : "Studio Task"}
                </div>
              </div>
                  ...
                  <div className="flex items-start gap-3.5">
                    ...
                    <div>
                      ...
                      <div className="text-xs md:text-sm text-neutral-500">
                        {task?.task?.projectID
                          ? `Project ID: ${task.task.projectID}`
                          : "Studio Task"}
                      </div>
                    </div>
                  </div>
