...
import type { ISODate, PlanPhase, WorkPackage, Milestone, PlanSnapshot } from "@/lib/plan-storage"
...
type Column =
  | { kind: "day"; iso: ISODate; width: number; weekday: number; isWeekend: boolean }
  | { kind: "month"; iso: ISODate; width: number; year: number; month: number; label: string; daysInMonth: number }
...
function seedPhases(today = new Date()): PlanPhase[] {
  ...
  return names.map((name, i) => {
    ...
    return {
      ...
      owner: ["Alex Morgan", "Jane Lee", "Mike Chen", "Sara Patel", "Chris Wood", "Dana Park"][i % 6] || "Alex Morgan",
...
export default function ProjectPlanPage() {
  ...
  useEffect(() => {
    ...
    if (loaded) {
      ...
      setWorkPackages([])
      setMilestones([
        { id: "m1", phaseId: seed[1].id, name: "Concept sign‑off", date: seed[1].end },
        { id: "m2", phaseId: seed[4].id, name: "Client approval", date: addDaysISO(seed[4].start, 3) },
      ])
    }
  }, [projectId])
  ...
  const gridTemplateColumns = useMemo(() => columns.map((c) => `${c.width}px`).join(" "), [columns])

  // View date range
  const viewStartISO: ISODate = useMemo(() => (columns.length ? columns[0].iso : ymd(new Date())), [columns])
  ...
  function boundsForPhase(start: ISODate, end: ISODate) {
    ...
    const width = Math.max(10, dateToPx(e) - dateToPx(s) + (columns[0].kind === "day" ? DAY_WIDTH : MONTH_WIDTH * 0.02))
    return { left, width }
  }
  ...
  const dragRef = useRef<{ id?: string; mode?: DragMode; startX?: number; origStart?: ISODate; origEnd?: ISODate }>({})
  ...
  function startDrag(e: React.PointerEvent, phaseId: string, mode: DragMode) {
    ...
    dragRef.current = { id: phaseId, mode, startX: e.clientX, origStart: p.start, origEnd: p.end }
  }
  function onMove(e: React.PointerEvent) {
    ...
    setPhases((prev) =>
      prev.map((p) => {
        ...
        if (a.mode === "move") {
          return { ...p, start: addDaysISO(a.origStart!, deltaDays), end: addDaysISO(a.origEnd!, deltaDays) }
  ...
  function extendIfNeeded() {
    ...
    const atRight = scroller.scrollLeft + scroller.clientWidth > scroller.scrollWidth - pad
    ...
    if (zoom === "week") {
      ...
      if (atLeft) {
        ...
        requestAnimationFrame(() => {
          if (scrollRef.current) scrollRef.current.scrollLeft += 4 * 7 * DAY_WIDTH
        })
      }
    } else {
  ...
  function onFit() {
    const scroller = scrollRef.current
    if (!scroller) return
    scroller.scrollLeft = Math.max(0, totalWidth / 2 - scroller.clientWidth / 2)
  }
  function onAutoLayout() {
    setPhases((prev) => {
      const sorted = [...prev].sort((a, b) => parseISO(a.start).getTime() - parseISO(b.start).getTime())
    ...
    toast({ title: "Auto‑layout applied", description: "Phases spaced while preserving duration." })
  }
  function onSavePlan() {
    const ok = savePlan(projectId, { phases, workPackages, milestones } as PlanSnapshot)
    toast({ title: ok ? "Plan saved" : "Save failed", description: ok ? "Saved locally." : "Please retry." })
  }
  ...
  function addPhase() {
    const newPhase: PlanPhase = {
      ...
      end: addDaysISO(addDaysISO(phases[phases.length - 1]?.end || ymd(new Date()), 3), 14),
      owner: "Alex Morgan",
      progress: 0,
      risk: "Low",
      expanded: true,
  ...
  return (
    <main className="flex-1 bg-gray-50">
      <div className="container mx-auto px-4 md:px-6 py-6 md:py-8 space-y-4 md:space-y-6">
        ...
        <div role="region" aria-label="Plan controls" className="flex flex-wrap items-center gap-2.5 py-2 md:py-3">
          {/* Chip toggles */}
            ...
            {(["week", "month", "quarter"] as Zoom[]).map((z) => (
              <button
                ...
                className={cn(
                  ...
                  zoom === z ? "bg-gray-900 text-white" : "text-gray-700 hover:bg-gray-50",
                )}
                onClick={() => setMode(z)}
                aria-pressed={zoom === z}
              >
          ...
          <div className="ml-auto flex items-center gap-2">
            <Button size="sm" variant="outline" className="h-9 bg-transparent" onClick={onFit}>
              <Maximize2 className="h-4 w-4 mr-2" />
              Fit
            </Button>
            <Button size="sm" variant="outline" className="h-9 bg-transparent" onClick={onAutoLayout}>
        ...
        {!hasPhases ? (
          <Card className="border-gray-200">
            <CardContent className="p-12 text-center">
              <div className="text-gray-900 font-medium mb-2">No phases yet</div>
              <p className="text-gray-600 mb-4">Create the first phase to start planning.</p>
              <Button
                onClick={addPhase}
            ...
            <div className="grid" style={{ gridTemplateColumns: "360px 1fr" }}>
              ...
              <div className="bg-white border-r border-gray-200">
                <div className="px-5 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
                  <div className="text-sm font-medium text-gray-700">Phases & work packages</div>
                  <Button
                    size="sm"
                ...
                <div>
                  {phases.map((p, idx) => {
                    const phaseWPs = workPackages.filter((w) => w.phaseId === p.id)
                    ...
                    return (
                      <div key={p.id} className="border-b border-gray-100">
                        ...
                        <div
                          ...
                          onMouseLeave={() => setHoveredPhaseId((id) => (id === p.id ? null : id))}
                        ...
                        >
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation()
                              setPhases((prev) =>
                                prev.map((x) => (x.id === p.id ? { ...x, expanded: !x.expanded } : x)),
                              )
                            }}
                            aria-label={p.expanded ? "Collapse phase" : "Expand phase"}
                          >
                            <ChevronDown
                              className={cn("h-4 w-4 transition-transform", p.expanded ? "rotate-0" : "-rotate-90")}
                            />
                          </button>

                          <div
                            className="h-8 w-1.5 rounded-full"
                            style={{ backgroundColor: theme.base, opacity: 0.9 }}
                          />
                          ...
                          <Avatar className="h-8 w-8">
                            <AvatarFallback className="text-[11px]">{initials(p.owner) || "PM"}</AvatarFallback>
                          </Avatar>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                              <DropdownMenuItem onClick={() => setDrawerPhaseId(p.id)}>
                              ...
                              <DropdownMenuItem
                                ...
                                onClick={(e) => {
                                  ...
                                  setWorkPackages((prev) => prev.filter((x) => x.phaseId !== p.id))
                                  setMilestones((prev) => prev.filter((x) => x.phaseId !== p.id))
                                  if (drawerPhaseId === p.id) setDrawerPhaseId(null)
                                }}
                              >
                        ...
                        {p.expanded &&
                          phaseWPs.map((w) => (
                            <div key={w.id} className="pl-14 pr-5 py-3 border-t border-gray-100">
                              <div className="grid grid-cols-5 items-center gap-2">
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setWorkPackages((prev) =>
                                      prev.map((x) => (x.id === w.id ? { ...x, name: e.target.value } : x)),
                                    )
                                  }
                                />
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setWorkPackages((prev) =>
                                      prev.map((x) => (x.id === w.id ? { ...x, start: e.target.value } : x)),
                                    )
                                  }
                                />
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setWorkPackages((prev) =>
                                      prev.map((x) => (x.id === w.id ? { ...x, end: e.target.value } : x)),
                                    )
                                  }
                                />
                                <div className="flex items-center justify-end">
                                  <Button
                                    ...
                                    variant="ghost"
                                    className="h-8 w-8"
                                    onClick={() => setWorkPackages((prev) => prev.filter((x) => x.id !== w.id))}
                                  >
              ...
              <div className="relative">
                <div className="overflow-x-auto" ref={scrollRef} onScroll={extendIfNeeded}>
                  ...
                  <div style={{ width: `${totalWidth}px` }}>
                    ...
                    <div className="sticky top-0 z-10 bg-gray-50 border-b border-gray-200">
                      ...
                      <div className="grid border-b border-gray-200" style={{ gridTemplateColumns }}>
                        {columns[0]?.kind === "day"
                          ? Array.from({ length: Math.ceil(columns.length / 7) }).map((_, i) => (
                              <div
                                key={`wk-${i}`}
                    ...
                    <div>
                      {phases.map((row, idx) => {
                        ...
                        const { left, width } = boundsForPhase(row.start, row.end)
                        const showRowTasks = showTasks && (hoveredPhaseId === row.id || drawerPhaseId === row.id)
                        const rowTasks = showRowTasks ? taskBarsForPhase(row.id) : []
                        return (
                          <div
                            key={row.id}
                            ...
                            onMouseLeave={() => setHoveredPhaseId((id) => (id === row.id ? null : id))}
                          >
                            <div className="relative h-full">
                              {/* Background vertical ticks – EXACT same grid as header */}
                              <div className="grid absolute inset-0" style={{ gridTemplateColumns }}>
                                {columns.map((c, i) => (
                                  <div
                                    key={c.kind === "day" ? c.iso : `${c.iso}-${i}`}
                                    className={cn(
                                      "border-r h-full",
                                      "border-gray-50",
                                      c.kind === "day" && parseISO(c.iso).getDay() === 1 && "bg-gray-50/40",
                                    )}
                                  />
                                ))}
                              </div>
                              ...
                              {todayLeft !== null && (
                                <div
                                  ...
                                  style={{ left: todayLeft, backgroundColor: "rgba(231, 76, 60, 0.7)" }}
                                  aria-hidden
                                />
                              )}
                              ...
                              {showPhases && width > 0 && left > -5000 && (
                                <div
                                  ...
                                  title={`${row.name} • ${formatShort(row.start)} – ${formatShort(row.end)}`}
                                >
                                  {/* Resize handles */}
                                  <div
                                    ...
                                    onPointerUp={endDrag}
                                  />
                                  <div className="px-3 text-xs font-medium truncate" style={{ color: theme.text }}>
                                    {row.name}
                                  </div>
                                  <div
                                ...
                                rowTasks.map((t) => {
                                  ...
                                  return (
                                    <div
                                      ...
                                      title={`${t.title}${t.dueDate ? ` • ${formatShort(t.dueDate)}` : ""}`}
                                    />
                                  )
                                })}
                            </div>
        ...
        <div className="text-xs text-gray-500 pt-1 md:pt-2">
          Week shows 12 weeks (84 days) and extends while you scroll. Month shows a full 12‑month year and extends
          across years. Quarter groups the 12 months into Q1–Q4 and also extends across years. The header and rows share
          the same grid to keep all lines perfectly aligned.
        </div>
      </div>

      {/* Phase Drawer */}
      <Sheet open={!!drawerPhase} onOpenChange={(o) => !o && setDrawerPhaseId(null)}>
        <SheetContent side="right" className="w-full sm:max-w-[520px]">
          {drawerPhase && (
            <>
              <SheetHeader>
                <SheetTitle>{drawerPhase.name}</SheetTitle>
                <SheetDescription>Phase details, milestones and linked tasks.</SheetDescription>
              </SheetHeader>
              ...
              <div className="mt-6 space-y-3">
                  ...
                  <CardContent className="p-4 grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setPhases((prev) =>
                            prev.map((p) => (p.id === drawerPhase.id ? { ...p, start: e.target.value } : p)),
                          )
                        }
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setPhases((prev) =>
                            prev.map((p) => (p.id === drawerPhase.id ? { ...p, end: e.target.value } : p)),
                          )
                        }
                      />
                    </div>
                    <div>
                      ...
                      <div className="font-medium">
                        {Math.max(1, diffDays(drawerPhase.start, drawerPhase.end) + 1)} days
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className="font-medium">{Math.round(drawerPhase.progress * 100)}%</div>
                    </div>
                    <div>
                      <div className="text-gray-500">Risk</div>
                      <div className="font-medium">{drawerPhase.risk || "—"}</div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="border-gray-200">
                  <CardContent className="p-4 space-y-3">
                    {milestones
                      ...
                      .map((m) => (
                        <div key={m.id} className="grid grid-cols-7 items-center gap-2">
                          <Input
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setMilestones((prev) =>
                                prev.map((x) => (x.id === m.id ? { ...x, date: e.target.value } : x)),
                              )
                            }
                          />
                          <Button
                ...
                <Card className="border-gray-200">
                  <CardContent className="p-4">
                    <div className="text-sm font-medium mb-2">Tasks in this phase</div>
                    <div className="text-xs text-gray-500">
                      Toggle "Show tasks" in the header to preview tasks inside the timeline.
                    </div>
                  </CardContent>
                </Card>
              </div>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      toast({
                        ...
                        description: "A spanning calendar entry would be created for this phase.",
                      })
                    }
                  >
