...
export default function ProjectTasksPage({
  ...
  React.useEffect(() => {
    ...
    if (taskData) {
      if (projectId) {
        const filterdTask = taskData?.data.filter(
        ...
        if (project?.phases) {
          setTasks(
            taskData &&
              ...
              updateTaskListFromPhases(filterdTask, project?.phases)
          );
        } else {
          setTasks(
            taskData && taskData?.data.length > 0 && updatetaskList(filterdTask)
          );
        }
      }
    }
                  ...
                  tasks?.map((col) => {
                    return (
                      <div
                        ...
                        onDrop={(e) =>
                          handleDrop(e, project?.phases ? col?.id : col?.name)
                        }
                        key={col?.name}
                        ...
                        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
                          <div className="flex items-center justify-between mb-4">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                openNewTask(
                                  project?.phases ? col?.id : col?.name
                                )
                              }>
