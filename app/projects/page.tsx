            ...
            <div className="bg-white border border-gray-200 rounded-lg p-1 flex gap-1">
              <button
                className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                  viewMode === "board" ? "text-white hover:text-white" : "text-gray-700 hover:text-white"
                }`}
                style={{
                  backgroundColor: viewMode === "board" ? "rgb(17, 24, 39)" : "transparent",
                }}
                onMouseEnter={(e) => {
                  if (viewMode !== "board") {
              ...
              <button
                className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                  viewMode === "table" ? "text-white hover:text-white" : "text-gray-700 hover:text-white"
                }`}
                style={{
                  backgroundColor: viewMode === "table" ? "rgb(17, 24, 39)" : "transparent",
                }}
                onMouseEnter={(e) => {
                  if (viewMode !== "table") {
                    e.currentTarget.style.backgroundColor = "rgb(17, 24, 39)"
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-start justify-between">
                        <h3 className="font-semibold text-ink line-clamp-1">{project.name}</h3>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="text-right">
                        <div className="font-medium text-ink">{project.budget}</div>
                        <div className="text-xs text-ink-muted">{project.spent} spent</div>
                      </div>
                    </div>
                      ...
                      <div className="flex -space-x-2">
                        ...
                        {project.team.length > 3 && (
                          <div className="w-6 h-6 rounded-full bg-greige-200 border-2 border-white flex items-center justify-center">
                            <span className="text-xs text-ink-muted">+{project.team.length - 3}</span>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <tbody className="divide-y divide-gray-200 text-sm">
                {filteredProjects.map((project, index) => (
                  <tr key={project.id} className="hover:bg-gray-50">
                    ...
                    <td className="py-4 px-4 text-sm text-ink">{project.client}</td>
                    <td className="py-4 px-4">
                      <Badge variant="outline" className={`text-xs ${getTypeColor(project.type)}`}>
                        <div className="flex items-center">
                          {getTypeIcon(project.type)}
                    ...
                    <td className="py-4 px-4">
                      <div className="text-sm">
                        <div className="font-medium text-ink">{project.budget}</div>
                        <div className="text-ink-muted">{project.spent} spent</div>
                      </div>
                    </td>
